      ...
      <div className="bg-[#14161a] border border-white rounded-[8px] h-[148px] w-full px-[21px] py-[5px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-[3px] w-[106px]">
          <div className="flex flex-col items-center justify-center">
            <div className="w-[72px] h-[72px] rounded-full overflow-hidden border border-white">
              <img src={avatarUrl || defaultAvatar} alt={name} className="w-full h-full object-cover" />
            </div>
            <div className="flex flex-col items-center mt-[2px]">
              <div className="text-[20px] font-bold text-white text-center leading-[20px] w-full">
                {name}
              </div>
              <div className="text-[10px] text-[#bfbdbd] text-center leading-[15px] w-full">
                @{username}
              </div>
            </div>
          </div>
...
export function RankSecondFrame({ name, username, level, avatarUrl, className = '', onClick, rankingType = 'level', score }: RankUserProps) {
  ...
  return (
    <div className={`flex flex-col items-center gap-[4px] ${className} cursor-pointer`} onClick={onClick}>
      <div className="bg-[#14161a] border border-white rounded-[8px] w-[128px] h-[128px] px-[21px] py-[5px] flex flex-col items-center justify-center gap-[4px]">
        <div className="flex flex-col items-center justify-center h-[85px] w-[82px]">
          <div className="w-[60px] h-[60px] rounded-full overflow-hidden border border-white flex-shrink-0">
            <img src={avatarUrl || defaultAvatar} alt={name} className="w-full h-full object-cover aspect-square" />
          </div>
          <div className="flex flex-col items-center pt-[4px]">
            <div className="text-[16px] font-bold text-white text-center leading-[20px] w-full">
              {name}
            </div>
            <div className="text-[10px] text-[#bfbdbd] text-center leading-[15px] w-full">
              @{username}
...
export function RankListFrame({ rank, name, username, level, avatarUrl, className = '', onClick, rankingType = 'level', score }: RankListFrameProps) {
  ...
  return (
    <div className={`bg-[#14161a] border border-white rounded-[8px] px-[13px] py-[7px] flex items-center gap-[12px] ${className} cursor-pointer`} onClick={onClick}>
      <div className="flex items-center gap-[10px] w-[279px]">
        <div className="text-[24px] font-bold text-white w-[49px]">{rank}st</div>
        <div className="w-[50px] h-[50px] rounded-full overflow-hidden border border-white">
          <img src={avatarUrl || defaultAvatar} alt={name} className="w-full h-full object-cover" />
        </div>
        <div className="flex flex-col">
          <div className="text-[20px] font-bold text-white leading-[20px]">
            {name}
          </div>
          <div className="text-[12px] text-[#bfbdbd] leading-[15px]">
            @{username}
          </div>
        </div>
      </div>
