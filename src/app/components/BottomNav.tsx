...
export default function BottomNav() {
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== 'undefined' && user?.id) {
        ...
        timer = setTimeout(() => setAvatarVersion(vb), 0);
      }
    } catch {}
    ...
    const onAvatarUpdated = (e: Event) => {
      try {
        ...
        const detail = evt.detail as { userId?: string } | undefined;
  ...
  return (
    <nav
      className='fixed bottom-0 left-0 right-0 bg-[#0b0c0f] border-t-4 border-white z-50'
              ...
              <span className='relative inline-flex items-center justify-center w-[48px] h-[48px]'>
                {isClient && user?.avatarUrl ? (
                  (() => {
                    ...
                    try {
                      if (avatarVersion)
                        src = `${src}${src.includes('?') ? '&' : '?'}v=${avatarVersion}`;
                    } catch {}
                    return (
