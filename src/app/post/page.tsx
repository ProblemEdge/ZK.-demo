...
export default function NewPostPage() {
  ...
  const { videoRef, canvasRef, fileInputRef, toggleFacingMode, stopStream, facingMode } = useCamera(
    isPhotoConfirmed,
    isLimitReached,
  ) as any;
  const { latitude, longitude, locationName, locating, setLocationName, handleUseCurrentLocation } =
    useLocation();
  const { isSending, sendingProgress, showPlane, loading, submitPost, handleCancelSending } =
    usePostSubmit({
      imageFile,
      title,
      caption,
  ...
  const handleCapture = useCallback(() => {
    ...
    ctx.save();
    // フロントカメラ（user）はプレビューと同じ向きに見せるため左右反転して描画
    if (facingMode === 'user') {
    ...
    <div className='min-h-screen bg-black'>
      {!isPhotoConfirmed ? (
        ...
        >
          {/* 左上の×ボタン */}
          <div
            className={`absolute left-4 top-4 z-10 transition-opacity duration-200 ${hideNonPostUi ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          ...
          <div
            className={`text-center text-white text-[28px] font-bold mb-4 transition-opacity duration-200 ${hideNonPostUi ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          >
            上にスワイプして投稿
          </div>
          ...
          <form onSubmit={handleSubmit} className='space-y-4 max-w-2xl mx-auto'>
            <div
              ...
              style={{
                transform: isSending
                  ? 'translateY(-150vh)'
                  : `translateY(${-swipeProgress * 624}px)`,
                touchAction: 'none',
            ...
            <div
              className={`mt-6 flex items-start justify-between gap-4 transition-opacity duration-200 ${hideNonPostUi ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            >
              {/* 左側：モード切替 */}
              <div className='flex flex-col gap-[18px] items-center w-[141px]'>
                ...
                >
                  <img
                    src={
                      isQuestMode ? '/icon/button_quest_post.svg' : '/icon/button_normal_post.svg'
                    }
              ...
              <div className='flex flex-col gap-[7px] w-[200px]'>
                ...
                <div className='flex items-center gap-[19px]'>
                  <span className='text-white text-[20px] font-bold'>
                    {visibilityScope === 'FRIENDS' ? '友達のみ' : '世界中　'}
                  </span>
                  <button
                    type='button'
                    onClick={() =>
                      setVisibilityScope((prev) => (prev === 'FRIENDS' ? 'PUBLIC' : 'FRIENDS'))
                    }
                    ...
                    <img
                      src={
                        visibilityScope === 'FRIENDS'
                          ? '/icon/toggle_under_water.svg'
                          : '/icon/toggle_under_blue.svg'
                      }
                      alt='トグル背景'
                    ...
                    <img
                      src={
                        latitude !== null && longitude !== null
                          ? '/icon/toggle_under_greensvg.svg'
                          : '/icon/toggle_under_gray.svg'
                      }
                      alt='トグル背景'
        ...
        <div className='fixed inset-0 z-[60] bg-black/90 flex flex-col items-center justify-center gap-6'>
          ...
          <div className='relative w-[140px] h-[80px] overflow-visible'>
            <img
              ...
              style={{
                transform: showPlane
                  ? 'translate(140px, -120px) rotate(-30deg)'
